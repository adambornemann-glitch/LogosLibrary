...
open StonesTheorem.Resolvent StonesTheorem.Yosida
...
/-  HELPER LEMMAS for Duhamel Estimate -/
-- Helper 0
...
-- Helper 1: adjoint of expBounded
...
-- Helper 2: expBounded of skew-self-adjoint is unitary
...
-- Helper 3: I • A is skew-self-adjoint when A is self-adjoint
...
theorem duhamel_estimate' (n : ℕ+) (t : ℝ) (φ : H) (hφ : φ ∈ gen.domain) :
    ‖U_grp.U t φ - expBounded (I • yosidaApproxSym gen hsa n) t φ‖ ≤
    |t| * ⨆ s ∈ Set.Icc 0 |t|, ‖gen.op ⟨U_grp.U s φ, gen.domain_invariant s φ hφ⟩ - yosidaApproxSym gen hsa n (U_grp.U s φ)‖ := by
  have h := duhamel_estimate gen hsa n t φ hφ
  have h_eq : (⨆ (s : Set.Icc 0 |t|), ‖gen.op ⟨U_grp.U s φ, gen.domain_invariant s φ hφ⟩ -
               yosidaApproxSym gen hsa n (U_grp.U s φ)‖) =
    ...
    have h_lhs : (⨆ (s : Set.Icc 0 |t|), ‖gen.op ⟨U_grp.U s φ, gen.domain_invariant s φ hφ⟩ -
                yosidaApproxSym gen hsa n (U_grp.U s φ)‖) =
                ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := by
      simp_rw [norm_gen_diff_constant gen hsa n _ φ hφ, ciSup_const]
    have h_rhs : (⨆ s ∈ Set.Icc 0 |t|, ‖gen.op ⟨U_grp.U s φ, gen.domain_invariant s φ hφ⟩ -
                yosidaApproxSym gen hsa n (U_grp.U s φ)‖) =
                ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := by
      have h_eq_const : ∀ s, s ∈ Set.Icc 0 |t| →
          ‖gen.op ⟨U_grp.U s φ, gen.domain_invariant s φ hφ⟩ - yosidaApproxSym gen hsa n (U_grp.U s φ)‖ =
          ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ :=
        fun s _ => norm_gen_diff_constant gen hsa n s φ hφ
      calc ⨆ s ∈ Set.Icc 0 |t|, ‖gen.op ⟨U_grp.U s φ, gen.domain_invariant s φ hφ⟩ -
              yosidaApproxSym gen hsa n (U_grp.U s φ)‖
          = ⨆ s ∈ Set.Icc 0 |t|, ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := by
            apply iSup_congr
            intro s
            apply iSup_congr
            intro hs
            exact h_eq_const s hs
        _ = ⨆ (_ : Set.Icc (0 : ℝ) |t|), ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := by
          simp only [Set.mem_Icc, ciSup_const]
          apply le_antisymm
          · apply ciSup_le
            intro s
            by_cases h : 0 ≤ s ∧ s ≤ |t|
            ...
            · simp only [not_and_or, not_le] at h
              simp_all only [Subtype.coe_prop, ciSup_const, nonempty_subtype, Set.mem_Icc, Set.nonempty_Icc,
                abs_nonneg, and_imp]
              obtain ⟨w, h_2⟩ := h_nonempty
              obtain ⟨left, right⟩ := h_2
              cases h with
              | inl h_2 =>
                simp_all only [isEmpty_Prop, not_and, not_le, IsEmpty.forall_iff, Real.iSup_of_isEmpty, norm_nonneg]
              | inr h_3 =>
                simp_all only [isEmpty_Prop, not_and, not_le, implies_true, Real.iSup_of_isEmpty, norm_nonneg]
          · have h0 : 0 ≤ (0 : ℝ) ∧ (0 : ℝ) ≤ |t| := ⟨le_refl 0, abs_nonneg t⟩
            calc ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖
                = ⨆ (_ : 0 ≤ (0 : ℝ) ∧ (0 : ℝ) ≤ |t|), ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := by
                  rw [ciSup_pos h0]
              _ ≤ ⨆ s, ⨆ (_ : 0 ≤ s ∧ s ≤ |t|), ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := by
                  apply le_ciSup_of_le _ (0 : ℝ)
                  exact le_refl _
                  · use ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖
                    intro x hx
                    simp only [Set.mem_range] at hx
                    ...
                    · simp only [not_and_or, not_le] at h
                      subst hs
                      simp_all only [Subtype.coe_prop, ciSup_const, nonempty_subtype, Set.mem_Icc, Set.nonempty_Icc,
                        and_imp, le_refl, abs_nonneg, and_self]
                      obtain ⟨w, h_2⟩ := h_nonempty
                      obtain ⟨left, right⟩ := h_2
                      cases h with
                      | inl h_2 =>
                        simp_all only [isEmpty_Prop, not_and, not_le, IsEmpty.forall_iff, Real.iSup_of_isEmpty,
                          norm_nonneg]
                      | inr h_3 =>
                        simp_all only [isEmpty_Prop, not_and, not_le, implies_true, Real.iSup_of_isEmpty, norm_nonneg]
        _ = ‖gen.op ⟨φ, hφ⟩ - yosidaApproxSym gen hsa n φ‖ := ciSup_const
    rw [h_lhs, h_rhs]
