...
Filename: SpectralTheory/RelThermo.lean
...
axiom functional_calculus_comm (E : Set ℝ → H →L[ℂ] H) (hE : IsSpectralMeasure E)
    (f g : ℝ → ℂ)
    (hf : ∃ M, ∀ s : ℝ, ‖f s‖ ≤ M)
    (hg : ∃ M, ∀ s : ℝ, ‖g s‖ ≤ M) :
...
theorem unitary_commutes_with_spectral (data : DiracSpectralData H)
  ...
  have hE : data.E B = FunctionalCalculus.boundedFunctionalCalculus data.E
    data.E_spectral.toIsSpectralMeasure (Set.indicator B 1) (indicator_bounded B) :=
    spectral_projection_eq_indicator data.E data.E_spectral.toIsSpectralMeasure B hB
  rw [hU, hE]
  -- Functions of the same operator commute
  exact functional_calculus_comm data.E data.E_spectral.toIsSpectralMeasure _ _ (exp_its_bounded t) (indicator_bounded B)
  ...
  have h_comm : E B (U_grp.U t ψ) = U_grp.U t (E B ψ) := by
    ...
    have h_op : U_grp.U t * E B = E B * U_grp.U t := by
      ...
      have hEB : E B = FunctionalCalculus.boundedFunctionalCalculus E
        hE.toIsSpectralMeasure (Set.indicator B 1) (indicator_bounded B) :=
        spectral_projection_eq_indicator E hE.toIsSpectralMeasure B hB
      rw [hU, hEB]
      exact functional_calculus_comm E hE.toIsSpectralMeasure _ _ (exp_its_bounded t) (indicator_bounded B)
...
theorem charge_conserved (data : DiracSpectralData H)
  ...
  rw [electron_number_conserved data hm t ψ,
      positron_number_conserved data hm t ψ]
...
theorem energy_variance_conserved {U_grp : OneParameterUnitaryGroup (H := H)}
  ...
  have h1 := spectral_moment_conserved gen hsa E hE 1 t ψ
  have h2 := spectral_moment_conserved gen hsa E hE 2 t ψ
...
theorem quantum_liouville {U_grp : OneParameterUnitaryGroup (H := H)}
    ...
    spectral_scalar_measure E (U_grp.U t ψ) hE.toIsSpectralMeasure B =
    spectral_scalar_measure E ψ hE.toIsSpectralMeasure B :=
  spectral_measure_invariant gen hsa E hE t ψ B hB
