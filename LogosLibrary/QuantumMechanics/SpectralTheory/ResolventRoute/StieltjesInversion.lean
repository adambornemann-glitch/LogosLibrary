...
Filename: ResolventRoute/StieltjesInversion.lean
-/
...
theorem stieltjes_inversion {U_grp : OneParameterUnitaryGroup (H := H)}
  ...
  have h_factor : (1 / Real.pi : ℂ) * ∫ t in Set.Icc a b,
    ...
    rw [show ∫ s, (1 / Real.pi) * (Real.arctan ((b - s) / ε) - Real.arctan ((a - s) / ε)) ∂μ =
         (1 / Real.pi) * ∫ s, (Real.arctan ((b - s) / ε) - Real.arctan ((a - s) / ε)) ∂μ from
