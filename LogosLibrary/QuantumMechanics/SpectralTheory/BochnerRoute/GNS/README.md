...
The logical flow from hypothesis to theorem:

```
IsContinuous f  (= PositiveDefinite + IsHermitian + ContinuousAt 0)
       │
       ▼
   ┌─────────────────────────────────┐
   │  Stage 1: PreHilbert/           │
   ...
   │  pdInner f : (ℝ →₀ ℂ)² → ℂ    │
   │  translate t : (ℝ →₀ ℂ) → …    │
   │  Sesquilinear, PSD, isometry   │
   │  Cauchy–Schwarz, variance est  │
   └───────────────┬─────────────────┘
                   │
                   ▼
   ┌─────────────────────────────────┐
   │  Stage 2: Completion/           │
   │                                 │
   │  N = ker(pdInner)  ──quotient─→ V = (ℝ →₀ ℂ)/N      │
   │                     ──complete─→ 𝓗 = V̂               │
   │  translate t  ──────descend───→ U₀(t) on V           │
   │               ──────extend────→ U(t) on 𝓗            │
   │  Strong continuity (ε/3)                              │
   │  ξ = ι(δ₀),  f(t) = ⟨ξ, U(t)ξ⟩_𝓗                   │
   └───────────────┬─────────────────┘
                   │
                   ▼
   ┌─────────────────────────────────┐
