...
This file proves that time dilation is not merely consistent with thermodynamics—
it is **derivable** from thermodynamics. Given the Ott temperature transformation
(T → γT, proven necessary in `Ott.lean`) and the thermal time ansatz (physical
time emerges from modular flow), time dilation t → t/γ follows as a theorem.

More remarkably, we prove the thermal time relation t = τ/T is **unique**: it is
the only Lorentz-covariant relationship between temperature and time. This transforms
Connes-Rovelli's 1994 proposal into a mathematical necessity.
...
time dilation from it combined with Ott

We now have two independent derivations of special relativistic time dilation:
1. Kinematic (Einstein): constancy of c
2. Thermodynamic (this file): Ott + thermal time uniqueness

Both encode the structure of Minkowski spacetime—one through light propagation,
one through thermal equilibrium.

## Main Results

### The Uniqueness Theorem
```
theorem thermalTime_unique : Any f(T,τ) satisfying
  (1) Positivity: f(T,τ) > 0
  (2) Linearity in τ: f(T, c·τ) = c·f(T,τ)
  (3) Lorentz covariance: f(γT, τ) = f(T,τ)/γ
must have the form f(T,τ) = c·τ/T for some constant c > 0.
```

This is the central result. The thermal time relation is not chosen—it is forced.

### Time Dilation as Theorem
```
theorem thermal_time_gives_time_dilation :
  thermalTime (γ*T) τ = thermalTime T τ / γ
```

A boosted observer sees temperature T' = γT (Ott). Their thermal time is
t' = τ/T' = τ/(γT) = t/γ. Time dilation emerges without postulating it.
...
The modular Hamiltonian K = H/T is a Lorentz scalar. The γ factors cancel.
This is why there is objective modular structure despite frame-dependent
...
| Theorem | Statement |
|---------|-----------|
| `functional_equation_solution` | g(γT) = g(T)/γ has unique solution g(T) = g(1)/T |
...
The uniqueness proof solves a multiplicative Cauchy equation. If g(T) satisfies:
- g(T) > 0 for T > 0
- g(γT) = g(T)/γ for all Lorentz factors γ ≥ 1

Then g(T)·T = g(1) for all T > 0, i.e., g(T) = constant/T.

The key lemma `lorentzGamma_surjective_ge_one` proves that every γ ≥ 1 is
physically realizable (by some velocity v with |v| < 1). This bridges the
abstract functional equation to physical Lorentz boosts.

### Why Ott Is Essential

The covariance condition f(γT, τ) = f(T,τ)/γ encodes:
- Temperature transforms: T → γT (Ott, proven necessary in `Ott.lean`)
- Time dilates: t → t/γ (what we derive here)
- Modular parameter invariant: τ → τ (algebraically defined)

If Landsberg were correct (T → T), the functional equation would be:
  f(T, τ) = f(T, τ)/γ
which forces f = 0 for γ ≠ 1. There would be no thermal time.

**Ott is required for thermal time to exist.**

### The Modular Hamiltonian

The ratio K = H/T is distinguished:
- H transforms as energy: H → γH
- T transforms via Ott: T → γT
- Therefore K → K (invariant)

This resolves the "problem of time" in quantum gravity:
- The Wheeler-DeWitt constraint H ≈ 0 kills the Hamiltonian as time generator
- But K = H/T can remain finite and generate modular flow
- Physical time is the modular parameter, not Hamiltonian evolution

### Connection to Jacobson

Jacobson (1995) derived Einstein's equations from thermodynamics at local
Rindler horizons. His derivation requires the Clausius relation δQ = TdS
to be Lorentz covariant. Theorem `rindler_thermodynamics_covariant` proves
this covariance holds under Ott:
- δQ → γδQ (energy flux transforms)
- T → γT (Ott)
- δS → δS (entropy invariant)
- Therefore δQ' = T'δS' ✓

Jacobson's derivation of GR is covariant because Ott is correct.

## Physical Consequences

### The Unruh Effect

The Unruh temperature T_U = a/(2π) has its 2π from modular periodicity:
```
theorem unruh_from_modular_period : unruhTemperature a = a / modular_period
```

The factor is not convention—it is the KMS periodicity of Tomita-Takesaki theory.

### Quantum Mechanics as T → 0

As temperature approaches zero:
- Thermal states approach the ground state
- Thermal fluctuations vanish
- Modular flow becomes unitary evolution

Ordinary quantum mechanics is the zero-temperature limit of thermal dynamics:
```
theorem thermal_to_ground_state_limit :
  Filter.Tendsto (fun β => thermalDensityMatrix β H) atTop (nhds 0)
```

### The Problem of Time

In quantum gravity, H|ψ⟩ = 0 (Wheeler-DeWitt) seems to forbid dynamics.
But modular flow σ_τ exists regardless:
- It is defined by the state, not by H
- It generates evolution parametrized by τ
- Physical time t = τ/T emerges from this flow

The Hamiltonian constraint kills H as generator. It does not kill K = H/T.

## Axioms

Two facts from Tomita-Takesaki theory are axiomatized pending full formalization:

| Axiom | Content |
|-------|---------|
| `kms_periodicity` | Thermal states have imaginary-time period β = 1/T |
| `kms_fixes_thermal_constant` | The KMS condition determines t = τ/T |

These are known mathematics (Takesaki 1970), not physical assumptions.

## File Structure
```
ThermalTime
├── thermalTime                        -- Definition: t = τ/T
├── thermal_time_gives_time_dilation   -- Time dilation derived
├── lorentzGamma_surjective_ge_one     -- Bridge lemma for uniqueness
├── satisfiesCovariance                -- The covariance condition
├── functional_equation_solution       -- g(γT) = g(T)/γ → g(T) = c/T
├── thermalTime_unique                 -- MAIN: uniqueness of t = τ/T
├── modularHamiltonian                 -- Definition: K = H/T
├── modular_hamiltonian_invariant      -- K is Lorentz scalar
├── unruhTemperature                   -- T_U = a/(2π)
├── LocalRindlerThermodynamics         -- Structure for Jacobson's argument
├── rindler_thermodynamics_covariant   -- Clausius relation is covariant
├── thermalDensityMatrix               -- ρ = e^{-βH}
├── thermal_to_ground_state_limit      -- QM as T → 0
└── thermal_time_consistency           -- Internal consistency check
```

## The Philosophical Import

For a century, we have asked: What is time? Why does it flow?

The thermal time framework answers:

**Time is the modular flow of states on algebras of observables.**

This is not metaphysics. It is mathematics:
- The modular flow exists (Tomita-Takesaki theorem)
- Its parametrization as physical time is unique (this file)
- It transforms correctly under Lorentz boosts (Ott + this file)
- It generates Einstein's equations (Jacobson)
- It reduces to quantum mechanics at T = 0 (this file)

Time is not a mystery to be contemplated. Time is a theorem to be proven.

## Dependencies

- `LogosLibrary.Relativity.LorentzBoost.Ott`: The Ott transformation T → γT
- `Mathlib.Analysis.SpecialFunctions.Log.Basic`: Logarithms and exponentials
- `Mathlib.Analysis.SpecialFunctions.Pow.Real`: Real powers for functional equation

## References

* [Connes, Rovelli, *Von Neumann algebra automorphisms and time-thermodynamics
  relation*][connes1994], Class. Quantum Grav. 11 (1994)
* [Jacobson, *Thermodynamics of Spacetime*][jacobson1995], PRL 75 (1995)
* [Rovelli, *The Statistical State of the Universe*][rovelli1993], CQG 10 (1993)
* [Takesaki, *Tomita's Theory of Modular Hilbert Algebras*][takesaki1970]
* [Unruh, *Notes on Black-Hole Evaporation*][unruh1976], PRD 14 (1976)

## Tags

thermal time, modular flow, Tomita-Takesaki, time dilation, Lorentz covariance,
Ott transformation, Connes-Rovelli, problem of time, quantum gravity
-/
namespace ThermalTimePedagogical
...
open ThermalTimePedagogical MinkowskiSpace RelativisticTemperature
