...
All errors and misinterpretations are solely the responsibility of the author (Adam Bornemann). This is an independent project. It has not been reviewed or endorsed by Erik Verlinde, Ted Jacobson, Alain Connes, Carlo Rovelli, Julian Barbour, Eric Weinstein, Roger Penrose, Lajos Diósi, Alain Chamseddine, or any other physicist or mathematician whose work is referenced.
...
4. **Jacobson's derivation** — Einstein's equations from thermodynamics, shown to produce Ott as output
...
Relativity/
├── SR/
│   └── MinkowskiSpacetime.lean       (467 lines)
├── GR/
│   ├── Kerr/                     (2,256 lines)
│   └── Forbidden/                (1,317 lines)
└── Thermodynamics/
    ├── Ott.lean                      (776 lines)
    └── Jacobson.lean                 (818 lines)
```
...
Proves that Jacobson's 1995 thermodynamic derivation of Einstein's field equations *produces* Ott as an output and is *incompatible* with Landsberg.
...
- Horizon structure, ergosphere properties, ring characterization in `Kerr/`
...
lake build LogosLibrary.Relativity.GR.Kerr
lake build LogosLibrary.Relativity.GR.Forbidden
