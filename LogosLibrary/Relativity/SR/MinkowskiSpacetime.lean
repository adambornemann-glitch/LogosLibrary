...
/-- The Lorentz factor: γ = 1/√(1 - v²)
    ...
    Defined for subluminal velocities |v| < 1.
...
/-- v² < 1 when |v| < 1 -/
lemma v_sq_lt_one (v : ℝ) (hv : |v| < 1) : v ^ 2 < 1 := by
  have := sq_abs v
  calc v ^ 2 = |v| ^ 2 := this.symm
    _ < 1 ^ 2 := by exact sq_lt_sq' (by grind) hv
    _ = 1 := one_pow 2

/-- 1 - v² > 0 when |v| < 1 -/
lemma one_minus_v_sq_pos (v : ℝ) (hv : |v| < 1) : 1 - v ^ 2 > 0 := by
  linarith [v_sq_lt_one v hv]

/-- √(1 - v²) > 0 when |v| < 1 -/
lemma sqrt_one_minus_v_sq_pos (v : ℝ) (hv : |v| < 1) :
    Real.sqrt (1 - v ^ 2) > 0 :=
  Real.sqrt_pos.mpr (one_minus_v_sq_pos v hv)

/-- The Lorentz factor is positive: γ > 0 -/
theorem lorentzGamma_pos (v : ℝ) (hv : |v| < 1) :
    lorentzGamma v hv > 0 := by
  unfold lorentzGamma
  exact div_pos one_pos (sqrt_one_minus_v_sq_pos v hv)

/-- γ ≠ 0 (immediate from γ > 0) -/
theorem lorentzGamma_ne_zero (v : ℝ) (hv : |v| < 1) :
    lorentzGamma v hv ≠ 0 :=
  ne_of_gt (lorentzGamma_pos v hv)

/-- At rest, γ = 1 -/
theorem lorentzGamma_zero : lorentzGamma 0 (by norm_num) = 1 := by
  unfold lorentzGamma
  simp [Real.sqrt_one]

/-- γ > 1 when v ≠ 0 -/
theorem lorentzGamma_gt_one (v : ℝ) (hv : |v| < 1) (hv_ne : v ≠ 0) :
    lorentzGamma v hv > 1 := by
  unfold lorentzGamma
  have hv_sq_pos : v ^ 2 > 0 := pow_two_pos_of_ne_zero hv_ne
  have h1 : 1 - v ^ 2 < 1 := by linarith
  have h2 : 1 - v ^ 2 > 0 := one_minus_v_sq_pos v hv
  have h3 : Real.sqrt (1 - v ^ 2) < 1 := by
    calc Real.sqrt (1 - v ^ 2) < Real.sqrt 1 :=
          Real.sqrt_lt_sqrt (le_of_lt h2) h1
      _ = 1 := Real.sqrt_one
  have h4 : Real.sqrt (1 - v ^ 2) > 0 := sqrt_one_minus_v_sq_pos v hv
  calc 1 / Real.sqrt (1 - v ^ 2) > 1 / 1 := by
        exact one_div_lt_one_div_of_lt h4 h3
      _ = 1 := by ring

/-- γ ≠ 1 when v ≠ 0 -/
theorem lorentzGamma_ne_one (v : ℝ) (hv : |v| < 1) (hv_ne : v ≠ 0) :
    lorentzGamma v hv ≠ 1 :=
  ne_of_gt (lorentzGamma_gt_one v hv hv_ne)
