...
Filename: HopfFibration.lean
-/
import Mathlib.Algebra.Quaternion
import Mathlib.Analysis.SpecialFunctions.Trigonometric.Basic
import Mathlib.Tactic
import LogosLibrary.Superior.Strings.Quaternion  -- for normSq, qConj, etc.
import LogosLibrary.Superior.YangMills.DivisionAlgebra              -- for NDA type
/-!
=====================================================================
...
theorem mul_conj_re (q : Quaternion ℝ) : (q * qConj q).re = normSq q := by
  unfold qConj normSq; simp [Quaternion.re_mul]; ring
...
theorem qConj_mul_unit (q : Quaternion ℝ) (hq : normSq q = 1) :
    qConj q * q = 1 := by
...
noncomputable def quatHopfOutputNormSq (a b : Quaternion ℝ) : ℝ :=
...
theorem quatHopf_norm_identity (a b : Quaternion ℝ) :
...
theorem quatHopf_maps_sphere (a b : Quaternion ℝ)
...
/-- **S³ ACTION PRESERVES S⁷**
    ...
    Proof: ‖aq‖² + ‖bq‖² = ‖a‖²‖q‖² + ‖b‖²‖q‖²
         = (‖a‖² + ‖b‖²)‖q‖² = 1·1 = 1 -/
theorem s3_action_preserves_sphere (a b q : Quaternion ℝ)
...
/-- **S³ ACTION ON HOPF₀**: the real component is preserved.
    ...
    For unit q: = ‖a‖² - ‖b‖² -/
theorem s3_action_hopf₀ (a b q : Quaternion ℝ) (hq : normSq q = 1) :
...
/-- **S³ ACTION ON HOPF_Q**: the quaternionic component is preserved.
    ...
    - Unit inverse: q·q̄ = 1 -/
theorem s3_action_hopfQ (a b q : Quaternion ℝ) (hq : normSq q = 1) :
...
theorem s3_fiber_invariance (a b q : Quaternion ℝ) (hq : normSq q = 1) :
...
theorem s3_action_preserves_output_norm (a b q : Quaternion ℝ) (hq : normSq q = 1) :
...
theorem s3_action_free (a q : Quaternion ℝ)
...
theorem s1_fiber_preserves_quatHopf₀ (a b : Quaternion ℝ) (θ : ℝ) :
    quatHopf₀ (a * s1Embed θ) (b * s1Embed θ) = quatHopf₀ a b :=
  s3_action_hopf₀ a b (s1Embed θ) (s1Embed_unit θ)

theorem s1_fiber_preserves_quatHopfQ (a b : Quaternion ℝ) (θ : ℝ) :
...
theorem s1_fiber_persistence (a b : Quaternion ℝ) (θ : ℝ) :
    quatHopf₀ (a * s1Embed θ) (b * s1Embed θ) = quatHopf₀ a b ∧
    quatHopfQ (a * s1Embed θ) (b * s1Embed θ) = quatHopfQ a b :=
  s3_fiber_invariance a b (s1Embed θ) (s1Embed_unit θ)

/-- S¹ preserves S⁷ -/
theorem s1_action_preserves_sphere (a b : Quaternion ℝ) (θ : ℝ)
