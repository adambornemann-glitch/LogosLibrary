...
Filename: SplitMechanics/ItFromSplit.lean
-/
...
structure GNSTriple (A : Type u) [CStarAlgebra A] (H : Type u) where
  inner : H → H → ℂ
  repr : A → H → H
  Ω : H
...
/-- The GNS Theorem. -/
axiom gns_construction {A : Type u} [CStarAlgebra A] (ω : State A) :
    ∃ (H : Type u) (G : GNSTriple A H), ∀ a, ω a = G.inner G.Ω (G.repr a G.Ω)

/-- The Born rule in algebraic form: ω(a) = ⟨Ω, π(a)Ω⟩. -/
def HasBornRuleForm {A : Type u} [CStarAlgebra A] (ω : State A) : Prop :=
  ∃ (H : Type u) (G : GNSTriple A H), ∀ a, ω a = G.inner G.Ω (G.repr a G.Ω)
...
structure PentagonVertices (A : Type u) [CStarAlgebra A] [IsVonNeumannAlgebra A]
    (ω : State A) where
  ...
  /-Failed to infer universe levels in type of field `born`
  HasBornRuleForm.{u_1, ?u.15436} ω-/
...
theorem pentagon_theorem {A : Type u} [CStarAlgebra A] [IsVonNeumannAlgebra A]
    (ω : State A) (hf : ω.IsFaithful) (hn : ω.IsNormal)
